...
- **GitHub-hosted execution**: move pull-request validation, trusted release jobs, security scans, and the scheduled health probe to ephemeral `ubuntu-24.04` runners; workflow contracts enforce the hosted runner matrix.
...
- **Runner cleanup**: remove obsolete host-runner documentation, systemd units, and maintenance scripts; every workflow uses GitHub-hosted runners.
...
- **Top-level shape validation**: Hue entertainment-configuration responses now require an object-shaped root before reading `data`, so scalar bridge responses fail closed through the normal sanitized invalid-response path instead of generating an exception log.

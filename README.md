...
| **Scene Effect Preview** | Choose a color, **Solid**, breathing **Pulse**, hue-cycling **Rainbow**, or warm flickering **Candle** effect, bounded 25-400% animation speed, brightness, 1-30 second duration, and optional 0-30 second Fade in and Fade out to preview the default target (or the current mapping target). The plugin ramps from dark to the selected effect, refreshes animated frames at a bounded cadence, and can ramp back to dark within the same duration; captures and restores the selected lights automatically. Save scenes for reuse, or use **Duplicate Scene** to create a uniquely named editable variant that preserves every visual and transition setting. **Cancel Active Preview** requests a safe stop for either preview button (and page navigation does the same best-effort cleanup), while active playback still blocks overlap. |

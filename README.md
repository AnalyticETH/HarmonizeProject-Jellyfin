...
| `GET /HueSync/ScenePlaylists` | List credential-free saved-scene playlists, including scene references, bounded `stepDurationSeconds` overrides (`0` inherits each scene duration), repeat passes, canonical `playbackOrder` (`Sequential` or `Shuffle`), selected target IDs/default-target inclusion, target mode/label, and bounded total duration. |
...
| `POST /HueSync/ScenePlaylists` | Save or update a playlist with an optional stable `id`, bounded `name`, up to 20 `presetNames`, a parallel `stepDurationSeconds` array (each value 0-30; `0` inherits the referenced scene's duration), bounded `repeatCount` (1-10, default 1), `playbackOrder` (`Sequential` for saved order or `Shuffle` for a stable date-seeded order, default `Sequential`), and either the legacy default target/single `targetUserId`, `targetAllEnabledMappings: true`, or selected `targetUserIds` (up to 50 enabled mappings) with optional `includeDefaultTarget: true`; all references, per-step/repeated duration, playback order, and targets are validated before persistence. Renaming an existing playlist atomically updates scheduled cues that reference its old name. |
| `POST /HueSync/ScenePlaylists/{name}/Preview` | Preview a saved playlist through the restorative scene lifecycle for its configured repeat passes and playback order. Each step uses its saved scene's effect/transitions and effective hold duration (`stepDurationSeconds`, or the scene duration when the value is `0`). An optional `{ "targetUserId": "..." }`, `{ "targetAllEnabledMappings": true }`, or selected `{ "targetUserIds": ["..."], "includeDefaultTarget": true }` override the saved target for that run; the response includes canonical playback order, expanded per-pass steps with each saved position and effective duration, credential-free aggregate target outcomes, channel counts, and cleanup warnings. Shuffle uses the playlist ID, UTC date, and pass number for a stable retryable sequence. |

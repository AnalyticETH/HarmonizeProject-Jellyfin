...
| **Backup and Restore** | Export global settings, per-user profiles, saved scene effects, saved scene playlists with repeat passes and `Sequential`/`Shuffle` playback order, scheduled scene cues, and history-retention preferences—including Solid/Pulse/Rainbow/Candle/Temperature/Aurora/Fire/Ocean/Lightning/Starlight metadata, bounded 25-400% rates, optional fade-in/fade-out transitions and per-step fade curves, one-time dates, optional per-cue hold durations, priorities, recurring date windows, and portable Fixed/SolarNoon/Sunrise/Sunset/CivilDawn/CivilDusk/NauticalDawn/NauticalDusk/AstronomicalDawn/AstronomicalDusk timing with bounded offsets and decimal coordinates—as a credential-safe JSON document. **Validate Import** runs the same normalization, dependency, and full-configuration preflight without changing the server, including planned totals, active-playback readiness, and a credential-safe added/removed/changed/unchanged diff for every imported object collection. Import is atomic, preserves matching stored keys on the same server, and includes an in-page password-field wizard for explicit replacement keys during migrations. |
...
| `POST /HueSync/Preview` | Display a bounded Solid, Pulse, Rainbow, Candle, Temperature, Aurora, Fire, Ocean, Lightning, or Starlight scene effect and restore the selected lights. Request fields include `ipAddress`, `appKey`, `clientKey`, optional `userId`, `entertainmentAreaId`, optional `channelIds`, `effect` (`Solid`, `Pulse`, `Rainbow`, `Candle`, `Temperature`, `Aurora`, `Fire`, `Ocean`, `Lightning`, or `Starlight`; blank/omitted preserves Solid), `effectSpeedPercent` (25-400; 100 is neutral and Solid ignores it), `red`, `green`, `blue` (0-255 seed color), `brightnessPercent` (0-100), `durationSeconds` (1-30), optional `transitionSeconds`/`transitionOutSeconds` (0-30, with their sum no greater than the duration), and optional `transitionCurve` (`Linear`, `SmoothStep`, `EaseIn`, `EaseOut`, or `EaseInOut`; blank/omitted preserves Linear). Use `{ "targetUserIds": ["mapping-id-1", "mapping-id-2"], "includeDefaultTarget": true }` without direct target fields for a deliberate selected subset, or set `targetAllEnabledMappings: true` to run sequentially on the valid global target plus every distinct enabled custom mapping; selected/all modes use each target's configured channel profile and return credential-free `targetUserIds`, `includeDefaultTarget`, `targetResults`, aggregate channel counts, and independent cleanup warnings. Active playback must be stopped first; matching stored mapping credentials are resolved server-side and never returned to the browser. |
...
| `POST /HueSync/ColorPresets` | Save or update a named scene with `name`, `effect` (`Solid`, `Pulse`, `Rainbow`, `Candle`, `Temperature`, `Aurora`, `Fire`, `Ocean`, `Lightning`, or `Starlight`), `effectSpeedPercent` (25-400; 100 is neutral), RGB seed values, `brightnessPercent`, `durationSeconds`, and optional `transitionSeconds`/`transitionOutSeconds` (0-30, with their sum no greater than the scene duration); names are case-insensitive, omitted effects and speed remain Solid/100%, values are validated, and persistence failures leave the previous scene collection unchanged. |
...
| `GET /HueSync/ScenePlaylists` | List credential-free saved-scene playlists, including scene references, bounded `stepDurationSeconds` overrides (`0` inherits each scene duration), optional `stepBrightnessPercent` overrides (blank/null inherits each scene brightness), nullable `stepTransitionSeconds` and `stepTransitionOutSeconds` overrides (blank/null inherits each scene fade), nullable `stepTransitionCurves` (`Linear`, `SmoothStep`, `EaseIn`, `EaseOut`, or `EaseInOut`; blank/null inherits each scene curve), repeat passes, canonical `playbackOrder` (`Sequential` or `Shuffle`), selected target IDs/default-target inclusion, target mode/label, and bounded total duration. |
...
| `POST /HueSync/ScenePlaylists` | Save or update a playlist with an optional stable `id`, bounded `name`, up to 20 `presetNames`, a parallel `stepDurationSeconds` array (each value 0-30; `0` inherits the referenced scene's duration), optional parallel `stepBrightnessPercent` (0-100 or `null`; `null` inherits brightness), `stepTransitionSeconds` and `stepTransitionOutSeconds` arrays (0-30 or `null`; `null` inherits fade-in/fade-out), and optional `stepTransitionCurves` values (`Linear`, `SmoothStep`, `EaseIn`, `EaseOut`, `EaseInOut`, or `null`; `null` inherits the saved scene curve), bounded `repeatCount` (1-10, default 1), `playbackOrder` (`Sequential` for saved order or `Shuffle` for a stable date-seeded order, default `Sequential`), and either the legacy default target/single `targetUserId`, `targetAllEnabledMappings: true`, or selected `targetUserIds` (up to 50 enabled mappings) with optional `includeDefaultTarget: true`; all references, per-step/repeated duration, brightness, transitions, curves, playback order, and targets are validated before persistence. Renaming an existing playlist atomically updates scheduled cues that reference its old name. |
| `POST /HueSync/ScenePlaylists/{name}/Preview` | Preview a saved playlist through the restorative scene lifecycle for its configured repeat passes and playback order. Each step uses its saved scene's effect, effective brightness (`stepBrightnessPercent`, or the scene brightness when `null`), hold duration (`stepDurationSeconds`, or the scene duration when `0`), fade-in (`stepTransitionSeconds`, or the saved scene when `null`), fade-out (`stepTransitionOutSeconds`, or the saved scene when `null`), and fade curve (`stepTransitionCurves`, or the saved scene when `null`); transitions are clamped to fit the effective hold. An optional `{ "targetUserId": "..." }`, `{ "targetAllEnabledMappings": true }`, or selected `{ "targetUserIds": ["..."], "includeDefaultTarget": true }` override the saved target for that run; the response includes canonical playback order, expanded per-pass steps with each saved position, effective brightness, duration, transitions, curves, and cumulative `startOffsetSeconds`, credential-free aggregate target outcomes, channel counts, and cleanup warnings. Shuffle uses the playlist ID, UTC date, and pass number for a stable retryable sequence. |
| `POST /HueSync/ScenePlaylists/BulkPreview` | Preview up to 50 selected playlist IDs with `{ "playlistIds": ["..."], "targetUserId": "...", "targetAllEnabledMappings": true }` or the selected-target override `{ "playlistIds": ["..."], "targetUserIds": ["mapping-id"], "includeDefaultTarget": true }`. Omitting the nullable target override preserves each playlist's saved target and playback order; all selected playlists and scene references are preflighted before bridge activity, results remain credential-free, later playlists continue after runtime failures, and cancellation stops the remaining sequence safely. |
| `POST /HueSync/ScenePlaylists/{name}/Duplicate` | Create a uniquely named copy of a playlist with a new stable ID while preserving its saved scenes, per-step duration, brightness, fade-in, fade-out, and fade-curve overrides, repeat count, playback order, and target mode. |
...
| `GET /HueSync/Configuration/Export` | Download a credential-safe JSON backup containing global settings, per-user profile fields, target labels, credential-presence flags, the session/cue-history retention preferences, the recurring-automation pause preference, saved scene effects and effect speeds, saved scene playlists with repeat counts and `playbackOrder`, and recurring or one-time scene cues. Secret values and persisted history entries are never included. |

...
[![codecov](https://codecov.io/gh/AnalyticETH/HarmonizeProject-Jellyfin/graph/badge.svg)](https://codecov.io/gh/AnalyticETH/HarmonizeProject-Jellyfin)
...
| **Playlist Step Effect Overrides** | The optional `stepEffects` array is parallel to `presetNames`; each nullable value selects Solid, Pulse, Rainbow, Candle, Temperature, Aurora, Fire, Ocean, Lightning, or Starlight, while blank/null inherits the referenced scene effect. Effective effects are shown in preview, occurrence, and retained-history step plans and preserved through API CRUD, duplication, exports, and credential-free backup/restore. |
...
| `GET /HueSync/ScenePlaylists` | List credential-free saved-scene playlists, including scene references, bounded `stepDurationSeconds` overrides (`0` inherits each scene duration), optional `stepBrightnessPercent`, `stepEffects`, and `stepEffectSpeedPercent` overrides (blank/null inherits the saved scene), nullable fade-in/fade-out/curve overrides, repeat passes, canonical `playbackOrder` (`Sequential` or `Shuffle`), selected target IDs/default-target inclusion, target mode/label, and bounded total duration. `stepEffects` accepts Solid, Pulse, Rainbow, Candle, Temperature, Aurora, Fire, Ocean, Lightning, or Starlight. |

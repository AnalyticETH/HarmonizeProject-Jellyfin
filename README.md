...
| **Solid Color Preview** | Choose a color, brightness, 1-30 second duration, and optional 0-30 second Fade in and Fade out to preview the default target (or the current mapping target). The plugin ramps from dark to the selected scene, holds the target, then can ramp back to dark within the same duration; captures and restores the selected lights automatically. **Cancel Active Preview** requests a safe stop for either preview button (and page navigation does the same best-effort cleanup), while active playback still blocks overlap. |

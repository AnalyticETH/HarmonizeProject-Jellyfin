...
`POST /HueSync/Preview` and `POST /HueSync/ColorPresets` accept `effect: "Temperature"`, `effect: "Aurora"`, `effect: "Fire"`, `effect: "Ocean"`, `effect: "Lightning"`, and `effect: "Starlight"`. The server validates and canonicalizes each effect, and the stream tester generates the warm-to-cool, drifting aurora, red/amber Fire, blue/cyan Ocean, electric blue/white Lightning, or cool white/blue Starlight palette without changing the credential-free target-selection contract. Current-light capture can read one target or a selected/all-target set and seed the editor with a weighted aggregate sample.
...
| `GET /HueSync/ScenePlaylists` | List credential-free saved-scene playlists, including scene references, repeat passes, canonical `playbackOrder` (`Sequential` or `Shuffle`), selected target IDs/default-target inclusion, target mode/label, and bounded total duration. |
...
| `POST /HueSync/ScenePlaylists` | Save or update a playlist with an optional stable `id`, bounded `name`, up to 20 `presetNames`, bounded `repeatCount` (1-10, default 1), `playbackOrder` (`Sequential` for saved order or `Shuffle` for a stable date-seeded order, default `Sequential`), and either the legacy default target/single `targetUserId`, `targetAllEnabledMappings: true`, or selected `targetUserIds` (up to 50 enabled mappings) with optional `includeDefaultTarget: true`; all references, repeated duration, playback order, and targets are validated before persistence. Renaming an existing playlist atomically updates scheduled cues that reference its old name. |
| `POST /HueSync/ScenePlaylists/{name}/Preview` | Preview a saved playlist through the restorative scene lifecycle for its configured repeat passes and playback order. An optional `{ "targetUserId": "..." }`, `{ "targetAllEnabledMappings": true }`, or selected `{ "targetUserIds": ["..."], "includeDefaultTarget": true }` override the saved target for that run; the response includes canonical playback order, expanded per-pass steps with each saved position, credential-free aggregate target outcomes, channel counts, and cleanup warnings. Shuffle uses the playlist ID, UTC date, and pass number for a stable retryable sequence. |
...
| `POST /HueSync/ScenePlaylists/{name}/Duplicate` | Create a uniquely named copy of a playlist with a new stable ID while preserving its saved scenes, repeat count, playback order, and target mode. |

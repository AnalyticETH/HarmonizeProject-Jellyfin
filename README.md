...
| `POST /HueSync/Preview/CaptureCurrentColor` | Capture one persisted default target or enabled user mapping with `{ "targetUserId": "mapping-id" }` (blank selects the default bridge). The server resolves the stored App Key and effective channel profile, reads the selected area's current light states, converts xy or valid mirek values to averaged RGB, retains average brightness, and returns credential-free sample/count fields for the administrator editor. All-target selections, stale channel profiles, active playback, concurrent diagnostics, and incomplete color data are rejected or reported without changing bridge state or returning light IDs/keys. |

...
| `POST /HueSync/ColorPresets` | Save or update a named scene with `name`, `effect` (`Solid`, `Pulse`, `Rainbow`, or `Candle`), RGB seed values, `brightnessPercent`, `durationSeconds`, and optional `transitionSeconds`/`transitionOutSeconds` (0-30, with their sum no greater than the scene duration); names are case-insensitive, omitted effects remain Solid, and values are validated. |

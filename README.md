...
| `GET /HueSync/ColorPresets/{name}/Dependencies` | Inspect one saved scene's credential-free lifecycle dependencies before editing or deleting it. Returns `canDelete`, dependent playlist and direct scheduled-cue counts, playlist IDs/names with repeated reference counts, and cue IDs/names/enabled state; bridge credentials and target details are never returned. |

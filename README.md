...
| `GET /HueSync/SceneSchedules/Occurrences?limit=50&days=31&scheduleId=...` | Preview bounded upcoming cue occurrences in UTC and the cue-local wall clock, including each effective hold duration and applying one-time dates, daily/weekly/monthly-day/monthly-weekday/yearly recurrence, bounded recurrence intervals anchored by `startDate`, ordinal weekday matching, short-month clamping, timezone/DST rules, weekday masks, date windows, and exclusions without contacting the bridge. `limit` is capped at 50 per response and `days` at 366. |
| `GET /HueSync/SceneSchedules/Calendar?limit=50&days=31&scheduleId=...` | Download the same bounded upcoming occurrences as an RFC 5545 iCalendar feed with UTC event times, cue timezone metadata, recurrence and interval metadata, target/scene descriptions, and effective cue durations; `limit` is capped at 50 and `days` at 366, and no bridge credentials are included. |

...
| **Backup and Restore** | Export global settings, per-user profiles, and saved color scenes as a credential-safe JSON document. Import is atomic, preserves matching stored keys on the same server, and supports explicit replacement keys for migrations. |
...
| `POST /HueSync/Configuration/Import` | Atomically restore an export document. Matching stored global/mapping keys are preserved when omitted; explicit global or mapping keys may be supplied for migration, and invalid documents leave the current configuration unchanged. Active playback must be stopped first. |

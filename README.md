...
| **Playlist Step Overrides** | Each saved playlist step can override hold duration (0 inherits), brightness (blank inherits), fade-in, and fade-out (blank/null inherits) within bounded ranges. Effective transitions are clamped to the step hold; expanded preview, scheduled-occurrence, and history plans expose repeat/original positions plus cumulative `startOffsetSeconds`. |
...
| `POST /HueSync/ScenePlaylists/{name}/Duplicate` | Create a uniquely named copy of a playlist with a new stable ID while preserving its saved scenes, per-step duration, brightness, fade-in, and fade-out overrides, repeat count, playback order, and target mode. |

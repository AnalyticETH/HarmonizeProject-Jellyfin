...
| `POST /HueSync/ScenePlaylists` | Save or update a playlist with an optional stable `id`, bounded `name`, up to 20 ordered `presetNames`, bounded `repeatCount` (1-10, default 1), and either the default target, one enabled `targetUserId`, or `targetAllEnabledMappings: true`; all references, repeated duration, and targets are validated before persistence. |
